...
const Recommended = () => {
        ...
        fetch("http://www.api.technicaltest.quadtheoryltd.com/api/Item?page=1&pageSize=10")
            ...
            .then((data) => {
                if (data && data.Items && Array.isArray(data.Items)) {
                    ...
                    setRecommendedData(recommendedItems);
                } else {
    ...
    const handleNext = () => {
        ...
        setCurrentIndex((prevIndex) => Math.min(recommendedData.length - itemsPerPage, prevIndex + 1));
    };

    const isLeftArrowHighlighted = currentIndex > 0;
    const isRightArrowHighlighted = currentIndex < recommendedData.length - (isMobile ? 3 : 5);
 
    const onSubmit = (data) => {
        setRecommendedData([data, ...recommendedData]);
        setShowModal(false);
    };
    return (
